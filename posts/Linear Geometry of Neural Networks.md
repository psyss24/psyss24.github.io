...
As networks grow to include billions of parameters, calculating these hinges one by one becomes impossible. We need a way to move all those fences simultaneously. This is where **(Matrix Notation)[a compact mathematical language used to represent large grids of numbers and operations, allowing us to compute millions of neuron activations at once.](#)** comes in. It is essentially an inventory management system for the network's geometry.

  

We group all the slopes into a weight matrix ($\Omega$) and all the thresholds into a bias vector ($\beta$):
...
Every time the network learns it is just adjusting these matrices to slightly reorient the millions of hyperplanes. The goal is to move the joints and fences until the resulting piecewise patchwork perfectly matches the data whether that data represents pixels in an image or the volatility of a stock price.