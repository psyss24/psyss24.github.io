// --- Header Dot Opacity Fade on Blog Pages ---
(function() {
  ...
  // Get initial and target opacities based on theme
  ...
  // Find the element after which the fade should finish (the tags row)
  const tagRow = document.querySelector('.post-page .tag-container');
  const fadeStart = 0; // Start fade at top of page
  const fadeEnd = tagRow
    ? tagRow.getBoundingClientRect().bottom + window.scrollY
    : 120; // Fallback if not found

  function updateHeaderOpacity() {
    const { from, to } = getOpacities();
    const scrollY = window.scrollY;
    let t = 0;
    if (scrollY <= fadeStart) t = 0;
    else if (scrollY >= fadeEnd) t = 1;
    else t = (scrollY - fadeStart) / (fadeEnd - fadeStart);
    const opacity = from + (to - from) * t;
    header.style.setProperty('--dot-opacity', opacity.toFixed(3));
  }

  // Set initial value and on scroll
  updateHeaderOpacity();
  window.addEventListener('scroll', updateHeaderOpacity);
  window.addEventListener('resize', updateHeaderOpacity);

  // Also update on theme change (if you support theme toggling)
  const observer = new MutationObserver(updateHeaderOpacity);
